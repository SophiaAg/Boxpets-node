...
module.exports = class planosController {
    static async planos(req, res) {
        const account = await connection.query("SELECT ID_USUARIOS, users.name, users.email, users.perfil, users.perfilBase64, users.points, users.pass, follows.followers FROM users INNER JOIN follows ON users.id = follows.UserId WHERE users.id = ?", [req.session.userid]);
